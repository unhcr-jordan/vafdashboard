...
// Then append a path element for every bit of data you've just binded.
	...
	zoomed();
		  ...
		  g.attr("class", "geogg")
		  .selectAll("path")
		  .data(topojson.feature(geog, geog.objects.geog).features)
		  .enter()
		  .append("path")
...
function zoom_by(factor){
	...
	function highlight(area) {
		...
		d3.select("#areainfo").html(function(d,i){if (!isNaN(rateById[area]))  {return dvc.prefix + rateById[area] + "<span>" + dvc.unittext + "</span>"} else {return "Data unavailable"}});
